...
func init() {
	Command = &cli.Command{
		Name:    "stagehand",
		Usage:   "CLI for the stagehand API",
		Suggest: true,
		Version: Version,
		Flags: []cli.Flag{
			&cli.BoolFlag{
		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				UsageText:       "stagehand @manpages [-o stagehand.1] [--gzip]",
				Hidden:          true,
...
func generateManpages(ctx context.Context, c *cli.Command) error {
	...
	if c.Bool("text") {
		file, err := os.Create(filepath.Join(dir, "man1", "stagehand.1"))
		if err != nil {
			return err
		}
		defer file.Close()
	...
	if c.Bool("gzip") {
		file, err := os.Create(filepath.Join(dir, "man1", "stagehand.1.gz"))
		if err != nil {
			return err
		}
		defer file.Close()
